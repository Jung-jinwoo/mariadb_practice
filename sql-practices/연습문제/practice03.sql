...
select concat(first_name,' ',last_name), emp_no, title
from(
	select e.first_name, e.last_name, e.emp_no, t.title
	from employees e join titles t on e.emp_no=t.emp_no
	where t.to_date = '9999-01-01'
	order by t.to_date desc) as x
order by first_name;

-- 문제3. 
-- 전체 사원의 사번, 이름, 현재 부서를 이름 순서로 출력하세요.. 
select e.emp_no, concat(first_name,' ',last_name), dept_name
from employees e join (select distinct dp.dept_no, de.emp_no, dp.dept_name
		from departments dp join dept_emp de on dp.dept_no=de.dept_no) as d
