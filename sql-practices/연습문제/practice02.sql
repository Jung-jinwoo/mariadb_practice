...
from employees
where hire_date != '9999-01-01' 
 order by datediff(date_format(curdate(),'%Y년 %m월 %d일'),date_format(hire_date,'%Y년 %m월 %d일')) desc limit 1;
...
select date_format(curdate(),'%Y') - min(date_format(birth_date, '%Y')) as 최연장자, 
date_format(curdate(),'%Y') - max(date_format(birth_date, '%Y')) as 최연소자
from employees;